...
namespace tests::csv_import_pipeline {
  ...
  namespace neutral_csv_parser_suite {
    ...
    TEST(NeutralCSVParserTests, ParsesSimpleUnquotedCSV) {
      ...
      auto result = CSV::parse::maybe::text_to_table(csv_text);
...
"Bob",25,Gothenburg
  ...
  namespace csv_pipeline_composition_suite {
    ...
    TEST(CSVPipelineCompositionTests, ParsesRealNordeaCSVWithEncoding) {
      ...
      auto table_result = CSV::parse::maybe::text_to_table(csv_text);
  ...
  namespace account_statement_suite {
    ...
    TEST(AccountStatementTests, DetectColumnsWithHeaders) {
      ...
      // Detect columns
      auto mapping = account::statement::detect_columns_from_header(table.heading);
    ...
    TEST(AccountStatementTests, DetectColumnsFromData) {
      ...
      auto mapping = account::statement::detect_columns_from_data(table.rows);
    ...
    TEST(AccountStatementTests, InvalidDateHandledGracefully) {
      ...
      auto mapping = account::statement::detect_columns_from_header(table.heading);
      auto maybe_entry = account::statement::extract_entry_from_row(table.rows[1], mapping);
    ...
    TEST(AccountStatementTests, DetectBothAmountColumns) {
      ...
      auto filtered_table = account::statement::filter_outlier_boundary_rows(*maybe_table);

      // Detect columns from data
      auto mapping = account::statement::detect_columns_from_data(filtered_table.rows);
