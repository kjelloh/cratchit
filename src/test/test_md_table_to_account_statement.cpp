...
protected:
  ...
  static std::optional<CSV::MDTable<AccountID>> make_md_table_from_csv(
      ...
      char delimiter = ';') {
    auto maybe_table = CSV::parse::monadic::parse_csv(csv_text, delimiter);
    if (!maybe_table) {
      return std::nullopt;
    }
    return CSV::project::to_account_id_ed(*maybe_table);
  }
...
TEST_F(MDTableToAccountStatementTestFixture, SKVNewerMDTableToAccountStatement) {
  ...
  auto maybe_table = CSV::parse::monadic::parse_csv(sz_SKV_csv_20251120, ';');
