...
namespace tea {
  ...
  namespace detail {
    ...
    std::optional<app::Msg> MetronomeEventEmitter::poll() {
      if (expired()) {
        return app::sub_to_msg(m_descriptor, MetronomeEventDescriptor::payload_type{});
      }
      return std::nullopt;
    }
    ...
    std::optional<app::Msg> TestEventEmitter::poll() {
      ...
      if (call_counter++ % 60 == 0) {
        return app::sub_to_msg(TestEventDescriptor{}, TestEventDescriptor::payload_type{42});
      }
      return std::nullopt;
  ...
  void SubHandler::update(Subs const& subs) {
    ...
    auto active = m_active_subscriptions
      | std::views::transform([](auto const& e) { return e.first; })
      | std::ranges::to<Subs>();
